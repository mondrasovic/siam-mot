...
class EMMPredictor(nn.Module):
    def __init__(self, cfg):
        ...
        self.cls_tower = make_conv3x3(
            in_channels=in_channels, out_channels=in_channels,
            use_gn=True, use_relu=True, kaiming_init=False
        )
        self.reg_tower = make_conv3x3(
            in_channels=in_channels, out_channels=in_channels,
            use_gn=True, use_relu=True, kaiming_init=False
        )
        self.cls = make_conv3x3(
            in_channels=in_channels, out_channels=2, kaiming_init=False
        )
        self.center = make_conv3x3(
            in_channels=in_channels, out_channels=1, kaiming_init=False
        )
        self.reg = make_conv3x3(
            in_channels=in_channels, out_channels=4, kaiming_init=False
        )
