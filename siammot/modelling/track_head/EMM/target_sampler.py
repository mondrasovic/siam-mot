...
class EMMTargetSampler(object):
    ...
    def __init__(
        self, track_utils, matcher, propsals_per_image=256,
    ...
    @staticmethod
    def sample_examples(
        src_box: [BoxList], pair_box: [BoxList],
    ...
    def generate_pos_hn_example(self, proposals, gts):
        ...
        for src_gt, tar_gt, proposal in zip(src_gts, tar_gts, proposals):
            ...
            sampled_pos = self.sample_examples(
                pos_src_boxlist, pos_pair_boxlist,
                pos_tar_boxlist, num_pos
            )
            sampled_hn = self.sample_examples(
                hn_src_boxlist, hn_pair_boxlist,
    ...
    def generate_neg_examples(
        self, proposals: [BoxList], gts: [BoxList], pos_hn_boxes: [BoxList]
    ):
        """
    ...
    def __call__(self, proposals: [BoxList], gts: [BoxList]):
        ...
        track_source = [cat_boxlist([pos_hn, neg]) for (pos_hn, neg) in
            zip(pos_hn_src, neg_src)]
        track_pair = [cat_boxlist([pos_hn, neg]) for (pos_hn, neg) in
            zip(pos_hn_pair, neg_pair)]
        track_target = [cat_boxlist([pos_hn, neg]) for (pos_hn, neg) in
            zip(pos_hn_tar, neg_tar)]
