...
class EMMTargetSampler(object):
    ...
    def assign_matched_ids_to_proposals(self, proposals: BoxList, gts: BoxList):
        ...
        for proposal, gt in zip(proposals, gts):
            proposal_ids, proposal_labels = self.match_targets_with_iou(
                proposal, gt
            )
    ...
    @staticmethod
    def sample_examples(
        src_box: List[BoxList], pair_box: List[BoxList], tar_box: List[BoxList],
        num_samples
    ):
        """
        Sample examples
    ...
    def sample_boxlist(self, boxlist: BoxList, indices, num_samples):
        ...
        if num_samples == 0:
            ...
            sampled_boxlist = self.get_default_boxlist(
                boxlist, sampled_boxes, sampled_ids, sampled_labels
            )
    ...
    def generate_hn_pair(
        self, src_gt: BoxList, proposal: BoxList, src_h=None, proposal_h=None
    ):
        """
    ...
    def generate_pos_hn_example(self, proposals, gts):
        ...
        for src_gt, tar_gt, proposal in zip(src_gts, tar_gts, proposals):
            pos_src_boxlist, pos_pair_boxlist, pos_tar_boxlist = (
                [] for _ in range(3)
            )
            hn_src_boxlist, hn_pair_boxlist, hn_tar_boxlist = (
                [] for _ in range(3)
            )
            ...
            for i, src_id in enumerate(src_ids):
                ...
                pos_tar_boxes = self.duplicate_boxlist(
                    _tar_box, len(pos_src_boxes)
                )

                hn_pair_boxes = self.generate_hn_pair(
                    _src_box, proposal, src_h[i], proposal_h
                )
                hn_src_boxes = self.duplicate_boxlist(
                    _src_box, len(hn_pair_boxes)
                )
                hn_tar_boxes = self.duplicate_boxlist(
                    _tar_box, len(hn_pair_boxes)
                )
            ...
            sampled_pos = self.sample_examples(
                pos_src_boxlist, pos_pair_boxlist, pos_tar_boxlist, num_pos
            )
            sampled_hn = self.sample_examples(
                hn_src_boxlist, hn_pair_boxlist, hn_tar_boxlist, num_hn
            )
    ...
    def generate_neg_examples(
        self, proposals: List[BoxList], gts: List[BoxList],
        pos_hn_boxes: List[BoxList]
    ):
        """
        ...
        for proposal, gt, pos_hn_box in zip(proposals, gts, pos_hn_boxes):
            ...
            neg_indices = (
                (proposal_ids == -1) & (objectness >= 0.3) & (proposal_h >= 5) &
                (proposal_w >= 5)
            )
            idxs = neg_indices.nonzero()

            neg_samples = min(
                idxs.numel(), self.proposals_per_image - len(pos_hn_box)
            )
    ...
    def __call__(self, proposals: List[BoxList], gts: List[BoxList]):
        ...
        pos_hn_src, pos_hn_pair, pos_hn_tar = self.generate_pos_hn_example(
            proposals, gts
        )
        neg_src, neg_pair, neg_tar = self.generate_neg_examples(
            proposals, gts, pos_hn_src
        )

        track_source = [
            cat_boxlist([pos_hn, neg])
            for (pos_hn, neg) in zip(pos_hn_src, neg_src)
        ]
        track_pair = [
            cat_boxlist([pos_hn, neg])
            for (pos_hn, neg) in zip(pos_hn_pair, neg_pair)
        ]
        track_target = [
            cat_boxlist([pos_hn, neg])
            for (pos_hn, neg) in zip(pos_hn_tar, neg_tar)
        ]
...
def make_emm_target_sampler(cfg, track_utils):
    matcher = Matcher(
    ...
    track_sampler = EMMTargetSampler(
        track_utils,
        matcher,
