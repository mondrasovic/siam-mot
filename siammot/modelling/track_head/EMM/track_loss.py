...
def select_cross_entropy_loss(pred, label):
    ...
    pred = pred.view(-1, 2)
    label = label.view(-1)
    pos = label.data.eq(1).nonzero().squeeze().to(device)
    neg = label.data.eq(0).nonzero().squeeze().to(device)
...
class EMMLossComputation(object):
    ...
    def compute_targets(self, locations, src_bbox, tar_bbox):
        ...
        s1 = _l > self.pos_ratio * ((tar_bbox[:, 2:3] - tar_bbox[:, 0:1]) /
                                    2).float()
        s2 = _r > self.pos_ratio * ((tar_bbox[:, 2:3] - tar_bbox[:, 0:1]) /
                                    2).float()
        s3 = _t > self.pos_ratio * ((tar_bbox[:, 3:4] - tar_bbox[:, 1:2]) /
                                    2).float()
        s4 = _b > self.pos_ratio * ((tar_bbox[:, 3:4] - tar_bbox[:, 1:2]) /
                                    2).float()
    ...
    def normalize_regression_outputs(src_bbox, regression_outputs):
        ...
        regression_outputs[:, :, 0] = (
            regression_outputs[:, :, 0] / half_src_box_w
        ) * 128
        regression_outputs[:, :, 1] = (
            regression_outputs[:, :, 1] / half_src_box_h
        ) * 128
        regression_outputs[:, :, 2] = (
            regression_outputs[:, :, 2] / half_src_box_w
        ) * 128
        regression_outputs[:, :, 3] = (
            regression_outputs[:, :, 3] / half_src_box_h
        ) * 128
    ...
    def __call__(
        ...
        return (
            self.loss_weight * cls_loss, self.loss_weight * reg_loss,
