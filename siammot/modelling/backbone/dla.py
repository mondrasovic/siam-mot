...
model_urls = {
    ...
    'dla60_res2net':
                ...
                '/download/v0.1-res2net/res2net_dla60_4s-d88db7f9.pth',
}
...
class DlaBasic(nn.Module):
    ...
    def __init__(
        ...
        batch_norm=FrozenBatchNorm2d, **_
    ):
        ...
        self.conv2 = Conv2d(
            planes, planes, kernel_size=3, stride=1, padding=dilation,
            bias=False, dilation=dilation
        )
...
class DlaBottleneck(nn.Module):
    ...
    def __init__(
        self, inplanes, outplanes, stride=1, dilation=1,
        ...
        if with_dcn:
            self.conv2 = DFConv2d(
                mid_planes, mid_planes, with_modulated_dcn=False,
...
class DlaBottle2neck(nn.Module):
    ...
    def __init__(
        self, inplanes, outplanes, stride=1, dilation=1, scale=4,
        ...
        self.conv1 = Conv2d(
            inplanes, mid_planes * scale, kernel_size=1, bias=False
        )
        ...
        self.conv3 = Conv2d(
            mid_planes * scale, outplanes, kernel_size=1, bias=False
        )
        self.bn3 = batch_norm(outplanes)
        self.relu = nn.ReLU(inplace=True)
...
class DlaTree(nn.Module):
    def __init__(
        ...
        cargs = dict(
            ...
            batch_norm=batch_norm, with_dcn=with_dcn
        )
        if levels == 1:
            ...
            self.tree1 = DlaTree(
                levels - 1, block, in_channels, out_channels, stride,
                root_dim=0, **cargs
            )
            self.tree2 = DlaTree(
                levels - 1, block, out_channels, out_channels,
                root_dim=root_dim + out_channels, **cargs
            )
        if levels == 1:
            self.root = DlaRoot(
...
class DLA(nn.Module):
    def __init__(
        self, levels, channels, num_classes=1000, in_chans=3, cardinality=1,
        base_width=64,
        ...
        if len(dcn_config) != 6:
            raise ValueError(
                "Deformable configuration is not correct, "
                "every level should specifcy a configuration."
            )
        
        self.base_layer = nn.Sequential(
            Conv2d(
                in_chans, channels[0], kernel_size=7, stride=1, padding=3,
                bias=False
            ),
        ...
        self.level0 = self._make_conv_level(
            channels[0], channels[0], levels[0], batch_norm=batch_norm
        )
        self.level1 = self._make_conv_level(
            channels[0], channels[1], levels[1], stride=2, batch_norm=batch_norm
        )
        cargs = dict(
            cardinality=cardinality, base_width=base_width,
            root_residual=residual_root, batch_norm=batch_norm
        )
        self.level2 = DlaTree(
...
def dla_46_c(dcn_config, feature_only=True, batch_norm=FrozenBatchNorm2d):
    model = DLA(
        [1, 1, 1, 2, 2, 1], [16, 32, 64, 64, 128, 256],
        block=DlaBottleneck,
        batch_norm=batch_norm,
        feature_only=feature_only,
        dcn_config=dcn_config
...
def dla_60(dcn_config, feature_only=True, batch_norm=FrozenBatchNorm2d):
    model = DLA(
        [1, 1, 1, 2, 3, 1], [16, 32, 128, 256, 512, 1024],
        block=DlaBottleneck,
        batch_norm=batch_norm,
        feature_only=feature_only,
        dcn_config=dcn_config
