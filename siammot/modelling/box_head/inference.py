...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, boxes):
        ...
        for i, (prob, boxes_per_img, image_shape, _box) in enumerate(
            zip(class_prob, proposals, image_shapes, boxes)
        ):
            ...
            if _box.has_field('ids'):
                ...
                # deafult id is -1
                ids = torch.zeros(
                    (len(_box), ), dtype=torch.int64, device=device
                ) - 1
            ...
            boxlist = self.prepare_boxlist(
                boxes_per_img, prob, image_shape, ids
            )
    ...
    def prepare_boxlist(boxes, scores, image_shape, ids):
        ...
        `scores` has shape (#detection, #classes), where each row represents
        a list
        of object detection confidence scores for each of the object classes
        in the
        dataset (including the background class). `scores[i, j]`` corresponds
        to the
    ...
    def create_empty_boxlist(self, device="cpu"):
        ...
        init_score = torch.zeros([
            0,
        ], dtype=torch.float32, device=device)
        init_ids = torch.zeros(([
            0,
        ]), dtype=torch.int64, device=device)
        empty_boxlist = self.prepare_boxlist(
            init_bbox, init_score, [0, 0], init_ids
        )
    ...
    def filter_results(self, boxlist, num_classes):
        ...
        # if we had multi-class NMS, we could perform this directly on the
        # boxlist
        ...
        result = [
            self.create_empty_boxlist(device=device)
        ...
        for j in range(1, num_classes):
            ...
            det_idx = ids_j < 0
            det_boxlist = BoxList(
                boxes_j[det_idx, :], boxlist.size, mode="xyxy"
            )
            ...
            if torch.any(track_idx > 0):
                track_boxlist = BoxList(
                    boxes_j[track_idx, :], boxlist.size, mode="xyxy"
                )
...
def make_roi_box_post_processor(cfg):
    ...
    postprocessor = PostProcessor(
        ...
        cls_agnostic_bbox_reg, bbox_aug_enabled, amodal_inference
    )
