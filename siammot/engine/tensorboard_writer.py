...
class TensorboardWriter(SummaryWriter):
    ...
    def __call__(self, iter, loss, loss_dict, images, targets):
        ...
        if get_world_size() < 2 or dist.get_rank() == 0:
            ...
            if iter == 1 or iter % 500 == 0:
                ...
                show_images = show_images.mul_(
                    self.model_std[None, :, None, None]
                ). \
                ...
                merged_image, bbox_in_merged_image = self.images_with_boxes(
                    show_images, targets
                )
                self.add_image_with_boxes(
                    'ground truth', merged_image, bbox_in_merged_image, iter
                )
    ...
    def images_with_boxes(self, images, boxes):
        ...
        for img_idx in range(batch):
            ...
            merged_image[:, row * height:(row + 1) * height,
        ...
        bbox_in_merged_image = np.array(
            list(itertools.chain(*bbox_in_merged_image))
        )
