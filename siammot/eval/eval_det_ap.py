...
def eval_det_ap(
    ...
    all_scores = [
        [[] for _ in range(len(iou_threshold))] for _ in range(num_classes)
    ]
    all_pr_ious = [
        [[] for _ in range(len(iou_threshold))] for _ in range(num_classes)
    ]
    ...
    for class_id in range(num_classes):
        class_ap_matrix[class_id, :] = compute_AP(
            all_scores[class_id], all_pr_ious[class_id], all_gt_ious[class_id]
        )
...
def get_ap(
    vid_class_gt: DataSample,
    vid_class_pred: DataSample,
    filter_fn,
    eval_frame_idxs,
    iou_thresh=[0.5]
):
