...
from .eval_utils import bbs_iou, compute_AP, greedy_matching


def eval_det_ap(
    gt: list, pred: dict, class_table=None, data_filter_fn=None,
    iou_threshold=[0.5]
):
    """
    ...
    :param data_filter_fn: a callable function that filters out detections
    that are not considered during evaluation
    ...
    all_scores = [[[] for _ in range(len(iou_threshold))] for _ in
        range(num_classes)]
    all_pr_ious = [[[] for _ in range(len(iou_threshold))] for _ in
        range(num_classes)]
    all_gt_ious = [[[] for _ in range(len(iou_threshold))] for _ in
        range(num_classes)]
    ...
    for (vid_id, vid_gt) in tqdm(gt):
        ...
        for class_id in range(0, num_classes):
            ...
            # gt_class_entities = vid_gt.get_entities_with_label(class_table[
            # class_id])
            pred_class_entities = vid_pred.get_entities_with_label(
            ...
            vid_class_scores, vid_class_pr_ious, vid_class_gt_ious = \
                get_ap(
                    vid_class_gt, vid_class_pred, data_filter_fn,
                    eval_frame_idxs, iou_threshold
                )
    ...
    for class_id in range(num_classes):
        class_ap_matrix[class_id, :] = compute_AP(
            all_scores[class_id],
...
def get_ap(
    vid_class_gt: DataSample, vid_class_pred: DataSample, filter_fn,
    eval_frame_idxs, iou_thresh=[0.5]
):
    """
    :param vid_class_gt: the ground truths for a specific class,
    in DataSample format
    :param vid_class_pred: the predictions for a specific class,
    in DataSample format
    ...
    :param iou_thresh: the list of iou threshod that determines whether a
    detection is TP
    :returns
           vid_scores: the confidence for every predicted entity (a Python list)
           vid_pr_ious: the iou between the predicted entity and its matching
           gt entity (a Python list)
           vid_gt_ious: the iou between the gt entity and its matching
           predicted entity (a Python list)
    """
    ...
    for frame_idx in eval_frame_idxs:
        ...
        if filter_fn is not None:
            ...
            gt_entities, ignore_gt_entities = filter_fn(
                gt_entities, meta_data=vid_class_gt.metadata
            )
            # Filter out predicted entities that overlaps with ignored gt
            # entities
            pred_entities, ignore_pred_entities = filter_fn(
                pred_entities, ignore_gt_entities
            )
        
        # sort the entity based on confidence scores
        pred_entities = sorted(
            pred_entities, key=lambda x: x.confidence, reverse=True
        )
        ...
        for i, _iou in enumerate(iou_thresh):
            ...
            pred_ious, gt_ious = greedy_matching(
                copy.deepcopy(iou_matrix), _iou
            )
            vid_scores[i] += scores
