...
def evaluate_recall(
    gt: List[AnnoEntity], pred: List[AnnoEntity], iou_thresh=0.5
):
    """
...
def compute_AP(scores, pred_ious, gt_ious):
    ...
    :param pred_ious: A list of iou of detections w.r.t the most matching
    ground truth bounding boxes
    :param gt_ious: A list of iou of ground truth bounding boxes w.r.t the
    most matching detections
    ...
    for i in range(len(scores)):
        precision, recall = precision_recall_curve(
            scores[i], pred_ious[i], gt_ious[i]
        )
