...
def load_motion_anno(
    ...
    """
    Load GluonCVMotionDataset format annotations for downstream training /
    testing
    """

    dataset = GluonCVMotionDataset(
        anno_file, root_path=dataset_folder, split_file=split_file
    )
...
def load_coco_anno(dataset_folder, anno_file):
...
def load_dataset_anno(cfg, dataset_key, set=None):
    ...
    if modality == 'video':
        dataset = load_motion_anno(dataset_folder, anno_file, split_file, set)
    elif modality == 'image':
        dataset = load_coco_anno(dataset_folder, anno_file)
...
def load_public_detection(cfg, dataset_key):
    ...
    try:
        public_detection = load_motion_anno(
            dataset_folder, 'anno_pub_detection.json', split_file
        )
    except:
        print(
            "The public detection is not ingested or provided in {}, "
            "skip public detection".format(
                os.path.join(
