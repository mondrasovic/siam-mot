...
class VideoDataset(data.Dataset):
    ...
    def __init__(
        ...
        :param frames_in_clip: the number of frames sampled in a video clip (
        for a training example)
        :param transforms: frame-level transformation before they are fed
    ...
    def get_video_clips(self, sampling_interval_ms=250):
        ...
        for (sample_id, sample) in tqdm(self.data.items()):
            ...
            clip_len_in_frames = max(
                self.frames_in_clip, int(self.clip_len / 1000. * sample.fps)
            )
            ...
            for idx in range(start_frame, end_frame, sampling_interval):
                ...
                # Only include video clips that have at least
