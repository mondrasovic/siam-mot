...
def sample_from_mot_csv(csv_path, fps, sample=None, mot17=True, has_gt=False):
    ...
    with open(csv_path, newline='') as f:
        ...
        for row in reader:
            ...
            # NOTE: Actually all classes that aren't Pedestrian have
            # confidence 0 and so should be ingested
...
def main(args, description="Initial ingestion", det_options=None, mot17=True):
    if mot17:
        if det_options is not None and not all(
            x in DET_OPTIONS for x in det_options
        ):
            raise ValueError(
                "Det options were {} but must be only: {}".format(
    ...
    out_dataset = GluonCVMotionDataset(
        out_filename, dataset_path, load_anno=False
    )
    metadata = {
    ...
    for det_option in det_options:
        for split_name, split_path in splits.items():
            ...
            for i, subdir in enumerate(subdirs):
                ...
                rel_base_dir = vid_path.replace(
                    out_dataset.data_root_path, ""
                ).lstrip(os.path.sep)
                ...
                sample = sample_from_mot_csv(
                    anno_path, fps, sample, mot17, has_gt
                )
