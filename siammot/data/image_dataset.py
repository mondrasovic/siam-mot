...
class ImageDataset(data.Dataset):
    def __init__(
        ...
        :param frames_per_image: how many image copies are generated from a
        single image
        :param amodal: whether to use amodal ground truth (no image boundary
        clipping)
        :param include_bg: whether to include the full background images
        during training
        """
        ...
        self._det_classes = [
            c['name'] for c in self.dataset.loadCats(self.dataset.getCatIds())
        ]

        # This is the mapping table of COCO labels
        self.json_category_id_to_contiguous_id = {
            v: i + 1
            for i, v in enumerate(self.dataset.getCatIds())
        }
    ...
    def _get_target(self, img, index):
        ...
        if len(labels) == 0:
            assert self._include_bg is True, "The image does not has ground " \
                                             "truth"
    ...
    def _check_load_bbox(self, entry, instance_id):
        ...
        entry_id = entry['id']
        entry_id = [entry_id
                   ] if not isinstance(entry_id, (list, tuple)) else entry_id
