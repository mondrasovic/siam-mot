...
class VisGenerator:
    ...
    def get_n_colors(n, colormap="gist_ncar"):
        ...
        # number of color transitions.
        # tab20 also seems like it would be a good option but it can only
        # show a max of 20 distinct colors.
        ...
        # Opencv expects bgr while cm returns rgb, so we swap to match the
        # colormap (though it also works fine without)
    ...
    def frame_vis_generator(self, frame, results: BoxList):
        ...
        for i, entity_id in enumerate(ids):
            ...
            cv2.putText(
                frame, str(entity_id), (x1 + 5, y1 + 40),
                cv2.FONT_HERSHEY_SIMPLEX, 1.5, color, thickness=3
            )
            # Draw black background rectangle for test
            cv2.rectangle(
                frame, (x1 - 5, y1 - 25), (x1 + text_width, y1), color, -1
            )
            cv2.putText(
                frame, '{}'.format(class_name), (x1 + 5, y1 - 5),
